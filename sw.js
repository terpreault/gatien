...
self.addEventListener("push",event=>{let data={title:"2LO 💧",body:"C’est l’heure de boire un peu d’eau.",icon:"./icons/icon-192.png",badge:"./icons/icon-192.png"};if(event.data){try{data={...data,...event.data.json()};}catch{data.body=event.data.text();}}const options={body:data.body,icon:data.icon||"./icons/icon-192.png",badge:data.badge||"./icons/icon-192.png",tag:data.tag||"2lo-water-reminder",renotify:true,data:{url:data.url||"./"}};event.waitUntil(self.registration.showNotification(data.title||"2LO 💧",options));});
